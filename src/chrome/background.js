...
function ahrefopen(info, tab) {
	...
	if (list != null) {
		...
		chrome.tabs.create({"url":list[n], "selected":false });
		n++;
		} while (n < list.length);
	} else {
		alert (extAlert);
...
chrome.extension.onRequest.addListener(
	function(request) {
		...
		if ( request == "ctrlkey" ) {
			ctrlchk = 1;
		} else if ( request == "shiftkey" ) {
			shiftchk = 1;
		} else if ( request == "ctrlshiftkey" ) {
			ctrlchk = 1;
			shiftchk = 1;
		} else {
			;
...
var ctrlchk  = 0;
var shiftchk = 0;
var extTitle = chrome.i18n.getMessage("extName");
var extAlert = chrome.i18n.getMessage("extAlertMessage");
var id       = chrome.contextMenus.create({"title": extTitle, "contexts":["selection"], "onclick": ahrefopen});
